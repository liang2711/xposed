...
int main(int argc, char* const argv[])
{
    ...
    if (!className.isEmpty()) {
        ...
        //添加参数为了在zygoteinit中开启systemserver
    ...
    //zygote模式 在整个Android生命周期中只会执行一次
    if (zygote) {
    //初始化 xposed框架
        isXposedLoaded = xposed::initialize(true, startSystemServer, NULL, argc, argv);
    //根据xposed框架初始化结果，执行原逻辑/修改的逻辑 在原版中是没有isXposedLoaded的  如果isXposedLoaded：true 跳转于 "de.robv.android.xposed.XposedBridge"
        runtimeStart(runtime, isXposedLoaded ? XPOSED_CLASS_DOTS_ZYGOTE : "com.android.internal.os.ZygoteInit", args, zygote);
    } else if (className) {//appliction模式 为了创建应用的
        isXposedLoaded = xposed::initialize(false, false, className, argc, argv);
        //isXposedLoaded:true "de.robv.android.xposed.XposedBridge$ToolEntryPoint"
