...
static bool init() {
    shared = (MemBasedState*) mmap(NULL, sizeof(MemBasedState), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);//申请空间
...
static inline void waitForIdle() {
    pthread_mutex_lock(&shared->workerMutex);//互斥锁 在这个线程中一直获取执行权
...
bool startAll() {
    ...
    if ((pid = fork()) < 0) {
        ...
        systemService();//子进程调用 fork()会返回两次第一次是父进程的pid，第二次是fork出来的子进程pid会返回的是0
