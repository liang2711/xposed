...
/** Handle special command line options.  这应该是app_main.cpp替换后 在调用时会设置的参数
    然后检查app_process运行程序时会不会有这些参数
 */
bool handleOptions(int argc, char* const argv[]) {
    parseXposedProp();
      //strcmp ==->0 s1>s2->+ s1<s2->-
      //判断argv已经有了就直接退出
...
bool initialize(bool zygote, bool startSystemServer, const char* className, int argc, char* const argv[]) {
    ...
    if (startSystemServer) {
        ...
        xposed::logcat::start();
#if XPOSED_WITH_SELINUX//如果XPOSED_WITH_SELINUX是预处理就成立
...
#if XPOSED_WITH_SELINUX
    // Don't let any further forks access the Zygote service 不要让更多的fork进入Zygote服务
...
/** Print information about the used ROM into the log  打印系统变量*/
void printRomInfo() {
...
void parseXposedProp() {
    ...
    while (fgets(buf, sizeof(buf), fp) != NULL) {
        char* key = buf;
        // Ignore leading spaces for the key 去掉空格、制表符、换行符等
        ...
        // Find the key/value separator寻找第一个=的位置
...
void disableXposed() {
    int fd;
    // FIXME add a "touch" operation to xposed::service::membased 创建conf/disabled文件
...
/** Add XposedBridge.jar to the Java classpath.   the classpath is a system environment variable */
...
/** Callback which checks the loaded shared libraries for libdvm/libart. 加载系统使用的so包的名字 */
...
void onVmCreated(JNIEnv* env) {
    ...
    if (!determineRuntime(&xposedLibPath)) {//查阅获取那个的libxposed.so 有dalvik and art
        ALOGE("Could not determine runtime, not loading Xposed");
        return;
    }

    // Load the suitable libxposed_*.so for it 加载成功后获得so文件的handle
    ...
    // Initialize the library 获得在so中的xposedInitLib
...
void setProcessName(const char* name) {
    memset(argBlockStart, 0, argBlockLength);//在argBlockStart的地址+argBlockLength都赋值于0
